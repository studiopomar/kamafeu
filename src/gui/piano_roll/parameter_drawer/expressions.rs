                ...
                |ui| {
                            ...
                            egui::ScrollArea::vertical()
                                ...
                                .show(ui, |ui| {
                                    ...
                                    let param_tabs = [
                                        ...
                                        (
                                            lang.tr("Fade-In (p2)", "Fade-In (p2)"),
                                            ParameterTab::EnvelopeFadeIn,
                                        ),
                                        (
                                            lang.tr("Fade-Out (p5)", "Fade-Out (p5)"),
                                            ParameterTab::EnvelopeFadeOut,
                                        ),
                                        (
                                            lang.tr("Crossfade (ovl)", "Crossfade (ovl)"),
                                    ...
                                    for (p_name, tab_val) in param_tabs {
                                        ...
                                        if tab_val == ParameterTab::Timbre {
                                            response.clone().on_hover_text(lang.tr(
                                                "Clique nas bolinhas ou arraste pelas notas. Botão direito restaura o padrão. A edição respeita a seleção.",
                                                "Click the circles or drag across notes. Right-click restores the default. Editing respects the selection.",
                                            ));
                                        }
                                        if response.clicked() {
                    ...
                    for (note_index, note) in notes.iter().enumerate() {
                        let norm = match state.selected_parameter {
                            ParameterTab::Timbre => unreachable!("Timbre has a discrete editor"),
                            ParameterTab::VoicebankPitch => unreachable!("Voicebank pitch has a discrete editor"),
                            ...
                            ParameterTab::EnvelopeFadeIn => (note.envelope.p2 / 100.0).clamp(0.0, 2.0) - 1.0,
                            ParameterTab::EnvelopeFadeOut => (note.envelope.p5 / 100.0).clamp(0.0, 2.0) - 1.0,
                            ParameterTab::EnvelopeCrossfade => (note.envelope.crossfade_ms / 100.0).clamp(0.0, 2.0) - 1.0,
                                ...
                                if click_t >= kf.start_ms - 20.0 && click_t <= kf.end_ms + 20.0 {
                                    if let Some(note) = notes.get_mut(kf.note_index) {
                                        match state.selected_parameter {
                                            ParameterTab::Timbre => {
                                                unreachable!("Timbre has a discrete editor")
                                            }
                                            ParameterTab::VoicebankPitch => unreachable!("Voicebank pitch has a discrete editor"),
                                            ParameterTab::Dynamics => {
                                ...
                                if click_t >= kf.start_ms - 20.0 && click_t <= kf.end_ms + 20.0 {
                                    if let Some(note) = notes.get_mut(kf.note_index) {
                                        match state.selected_parameter {
                                            ...
                                            ParameterTab::EnvelopeFadeIn => {
                                                note.envelope.p2 =
                                                    ((1.0 + norm) * 100.0).clamp(0.0, 300.0);
                                            }
                                            ParameterTab::EnvelopeFadeOut => {
                                                note.envelope.p5 =
                                                    ((1.0 + norm) * 100.0).clamp(0.0, 300.0);
                    ...
                    if graph_response.hovered() || is_drawing {
                        ...
                        let val_str = match state.selected_parameter {
                            ...
                            ParameterTab::EnvelopeFadeIn => {
                                let v = note_under_cursor
                                    .map(|n| n.envelope.p2)
                                    .unwrap_or((1.0 + hover_norm) * 100.0);
                                format!("Fade-In (p2) {:.1} ms", v)
                            }
                            ParameterTab::EnvelopeFadeOut => {
                                let v = note_under_cursor
                                    .map(|n| n.envelope.p5)
                                    .unwrap_or((1.0 + hover_norm) * 100.0);
                                format!("Fade-Out (p5) {:.1} ms", v)
                            }
                            ParameterTab::EnvelopeCrossfade => {
                                let v = note_under_cursor
                                    .map(|n| n.envelope.crossfade_ms)
                                    .unwrap_or((1.0 + hover_norm) * 100.0);
                                format!("Crossfade (ovl) {:.1} ms", v)
                            }
                            ParameterTab::VibratoLength => {
                                let v = note_under_cursor
