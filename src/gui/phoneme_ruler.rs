            ...
            ui.horizontal(|ui| {
                ...
                let pointer_pos = ui.input(|i| {
                    ...
                    let started_in_ruler = i
                        .pointer
                        .press_origin()
                        .is_some_and(|origin| strip_rect.contains(origin));

                    // A captured drag may leave the strip for edge scrolling, but
                    // pointer coordinates from another panel must not mutate the
                    // ruler unless the gesture really started here.
                    (strip_rect.contains(pos)
                        || (drag_active && is_primary_down && started_in_ruler))
                        .then_some(pos)
                });
                        ...
                        || state.dragging_envelope_pt.is_some())
                {
                    if let Some(pos) = pointer_pos {
                        ...
                        let left_edge = strip_rect.min.x + edge_margin;
                        let right_edge = strip_rect.max.x - edge_margin;
                ...
                // Zoom can briefly be zero/non-finite while the canvas is
                // being resized or a trackpad gesture is committed. Keep the
                // ruler interactive without allowing invalid screen geometry
                // to leak into note handles or bar positions.
                let px_per_ms = f64::from(state.px_per_ms).max(0.001).min(100.0);

                let safe_bpm = if bpm.is_finite() {
                    bpm.clamp(1.0, 1_000.0)
                } else {
                    120.0
                };
                let beat_ms = 60000.0 / safe_bpm;
                ...
                for (note_index, note) in notes.iter_mut().enumerate() {
                    let note_position_ms = if note.position_ms.is_finite() {
                        note.position_ms
                    } else {
                        0.0
                    };
                    let note_duration_ms = if note.duration_ms.is_finite() {
                        note.duration_ms.max(0.0)
                    } else {
                        0.0
                    };
                    let prior_end_ms = previous_note_end_ms;
                    let prior_lyric = previous_note_lyric.clone();
                    previous_note_end_ms = Some(note_position_ms + note_duration_ms);
                    previous_note_lyric = Some(note.lyric.clone());

                    let x_start =
                        ruler_rect.min.x + keyboard_width + (note_position_ms * px_per_ms) as f32
                            - timeline_scroll_x;
                    let x_end = x_start + (note_duration_ms * px_per_ms) as f32;
                        ...
                        if let Some(cached) = state.note_phonemes_cache.get(note_index) {
                            if !cached.is_empty() {
                                ...
                                if manual_parts.len() > 1 {
                                    ...
                                    vec![(lyric_trimmed.clone(), 0.0, note_duration_ms)]
                                }
                            }
                        } else {
                            let manual_parts: Vec<&str> = lyric_trimmed
                    ...
                    let oto_consonant = finite_or_zero(
                        state
                            .oto_consonant_cache
                            .get(note_index)
                            .copied()
                    ...
                    let oto_preutter = finite_or_zero(
                        state
                            .oto_preutter_cache
                            .get(note_index)
                            .copied()
                    ...
                    let oto_overlap = finite_or_zero(
                        state
                            .oto_overlap_cache
                            .get(note_index)
                            .copied()
                    ...
                    let active_consonant_ms = (base_consonant_ms
                        + finite_or_zero(note.expressions.consonant_timing_offset_ms))
                    .clamp(0.0, note_duration_ms.max(10.0) * 0.95);

                    let base_preutter_ms = oto_preutter * consonant_v_scale;
                    let active_preutter_ms = (base_preutter_ms
                        + finite_or_zero(note.expressions.preutter_offset_ms))
                    .max(0.0);

                    let base_overlap_ms = oto_overlap * consonant_v_scale;
                    let active_overlap_ms =
                        base_overlap_ms + finite_or_zero(note.expressions.overlap_offset_ms);
                            ...
                            if pos.x >= strip_rect.min.x - 10.0 && pos.x <= strip_rect.max.x + 10.0
                            {
                                ...
                                if active {
                                        ...
                                        && state.dragging_subphoneme_boundary.is_none()
                                    {
                                        ...
                                        let init_timeline_x = if let Some(cursor_pos) = pointer_pos
                                        {
                                            cursor_pos.x + timeline_scroll_x
                                        } else {
                                            draw_pos.x + timeline_scroll_x
                                        };
                                        state.dragging_phoneme_handle =
                                            Some((note_index, kind, init_timeline_x, init_val));
                                ...
                                ui.vertical(|ui| {
                                    ...
                                    if let Some(vb) = voicebank {
                                            ...
                                            vb.find_mapped_entry(&lyric_trimmed, &note.pitch)
                                        {
                                            ui.horizontal(|ui| {
                                                ui.label(
                                                    RichText::new("oto.ini:").strong().size(10.0),
                                                );
                                                ui.label(
                                                    RichText::new(&entry.alias)
                        ...
                        state.dragging_phoneme_handle
                    {
                        if drag_idx == note_index {
                            if let Some(pos) = pointer_pos {
                                let delta_px = (pos.x + timeline_scroll_x) - init_x;
