...
impl KamafeuStudioApp {
    ...
    pub(super) fn draw_no_phonemizer_warning_bubble(&mut self, ctx: &egui::Context) {
        if self.phonemizer_warning_dismissed {
            return;
        }

        if self.vocal_mode_params.phonemizer_mode != crate::phonemizer::PhonemizerMode::None {
            return;
        }
        ...
        let lang = self.config.language;
        let mut dismiss = false;
        let mut selected_new_mode = None;
        let mut open_singer_panel = false;
        let is_expanded = self.phonemizer_warning_expanded;
        let mut toggle_expanded = false;

        egui::Area::new(egui::Id::new("no_phonemizer_warning_bubble_area"))
            .anchor(egui::Align2::CENTER_BOTTOM, egui::vec2(0.0, -35.0))
            .order(egui::Order::Foreground)
            .interactable(true)
            .show(ctx, |ui| {
                let bg_color = egui::Color32::from_rgb(26, 22, 28);
                let border_color = egui::Color32::from_rgb(255, 175, 55);
                let text_main = egui::Color32::from_rgb(245, 240, 248);
                let text_sec = egui::Color32::from_rgb(195, 180, 200);

                egui::Frame::none()
                    .fill(bg_color)
                    .rounding(egui::Rounding::same(8.0))
                    .stroke(egui::Stroke::new(1.4, border_color))
                    .shadow(egui::epaint::Shadow {
                        offset: egui::Vec2::new(0.0, 4.0),
                        blur: 16.0,
                        spread: 1.0,
                    ...
                    .inner_margin(egui::Margin::symmetric(14.0, 10.0))
                    .show(ui, |ui| {
                        ui.set_max_width(if is_expanded { 500.0 } else { 420.0 });
                        ui.vertical(|ui| {
                            ui.horizontal(|ui| {
                                ui.label(
                                    egui::RichText::new("🗣️")
                                ...
                                ui.label(
                                    egui::RichText::new(lang.tr(
                                        "Nenhum fonemizador selecionado",
                                        "No phonemizer selected",
                                    ))
                                    .strong()
                                    .size(12.0)
                                    .color(border_color),
                                ...
                                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                    ...
                                    .fill(egui::Color32::from_rgb(45, 38, 50))
                                    .min_size(egui::vec2(18.0, 18.0));
                                    if ui.add(close_btn).on_hover_text(lang.tr("Dispensar aviso", "Dismiss warning")).clicked() {
                                        dismiss = true;
                                    }
                                });
                            });
                            ...
                            ui.label(
                                egui::RichText::new(lang.tr(
                                    "O modo atual é 'None' (letra literal). Selecione um fonemizador para conversão automática:",
                                    "Current mode is 'None' (direct lyric). Select a phonemizer for automatic phonetic conversion:",
                                ))
                                .size(10.0)
                                .color(text_sec),
                            );
                            ...
                            if !is_expanded {
                                ui.horizontal_wrapped(|ui| {
                                    ...
                                    let presets = [
                                        (crate::phonemizer::PhonemizerMode::BasicCV, "JA: CV"),
                                        (crate::phonemizer::PhonemizerMode::VCV, "JA: VCV"),
                                        (crate::phonemizer::PhonemizerMode::PortugueseBrapaVCCV, "PT: BRAPA"),
                                        (crate::phonemizer::PhonemizerMode::EnglishArpasing, "EN: Arpasing"),
                                    ];

                                    for (mode, label) in presets {
                                    ...
                                    let more_btn = egui::Button::new(
                                        egui::RichText::new(lang.tr("Mais...", "More..."))
                                            .size(9.5)
                                            .color(text_main),
                                    )
                                    .fill(egui::Color32::from_rgb(50, 42, 58))
                                    .min_size(egui::vec2(48.0, 20.0));

                                    if ui.add(more_btn).on_hover_text(lang.tr("Ver todos os fonemizadores", "View all phonemizers")).clicked() {
                                    ...
                                    .show(ui, |ui| {
                                        ...
                                        ui.horizontal_wrapped(|ui| {
                                            for (mode, label) in [
                                                ...
                                                if ui.small_button(egui::RichText::new(label).strong().color(egui::Color32::BLACK)).clicked() {
                                                    selected_new_mode = Some(mode);
                                                }
                                            }
                                        });
                                        ...
                                        ui.add_space(4.0);
                                        ui.label(egui::RichText::new(lang.tr("🇧🇷 Português:", "🇧🇷 Portuguese:")).strong().size(10.0).color(border_color));
                                        ui.horizontal_wrapped(|ui| {
                                            for (mode, label) in [
                                                (crate::phonemizer::PhonemizerMode::PortugueseBrapaVCCV, "PT: VCCV BRAPA (3.7)"),
                                                (crate::phonemizer::PhonemizerMode::PortugueseBrapaCVC, "PT: BRAPA CVC"),
                                                (crate::phonemizer::PhonemizerMode::PortugueseCVVC, "PT: CVVC"),
                                                (crate::phonemizer::PhonemizerMode::PortugueseVCV, "PT: VCV"),
                                                (crate::phonemizer::PhonemizerMode::PortugueseG2P, "PT: G2P"),
                                            ] {
                                                if ui.small_button(egui::RichText::new(label).strong().color(egui::Color32::BLACK)).clicked() {
                                                    selected_new_mode = Some(mode);
                                                }
                                        ...
                                        ui.add_space(4.0);
                                        ui.label(egui::RichText::new(lang.tr("🇺🇸 Inglês:", "🇺🇸 English:")).strong().size(10.0).color(border_color));
                                        ui.horizontal_wrapped(|ui| {
                                            for (mode, label) in [
                                                (crate::phonemizer::PhonemizerMode::EnglishArpasing, "EN: Arpasing"),
                                                (crate::phonemizer::PhonemizerMode::EnglishVCCV, "EN: English VCCV"),
                                                (crate::phonemizer::PhonemizerMode::EnglishG2P, "EN: G2P (Words)"),
                                            ] {
                                                if ui.small_button(egui::RichText::new(label).strong().color(egui::Color32::BLACK)).clicked() {
                                                    selected_new_mode = Some(mode);
                                                }
                                ...
                                ui.separator();
                                ui.horizontal(|ui| {
                                    if ui.button(egui::RichText::new(lang.tr("📁 Abrir Painel Lateral do Cantor", "📁 Open Singer Sidebar Panel")).size(10.0).color(text_main)).clicked() {
                                        open_singer_panel = true;
                                    }
                                    if ui.button(egui::RichText::new(lang.tr("Recolher", "Collapse")).size(10.0)).clicked() {
                                        toggle_expanded = true;
                                    }
                                });
                            }
        ...
        if toggle_expanded {
            self.phonemizer_warning_expanded = !self.phonemizer_warning_expanded;
        }

        if let Some(mode) = selected_new_mode {
            self.vocal_mode_params.phonemizer_mode = mode;
            self.project.phonemizer = Some(mode);
            for track in &mut self.project.tracks {
                track.phonemizer = Some(mode);
            }
            self.piano_roll_state.phoneme_cache.clear();
            self.piano_roll_state.note_phonemes_cache.clear();
            self.is_dirty = true;
            self.phonemizer_warning_dismissed = true;
            self.phonemizer_warning_expanded = false;
            self.transport_state.status_message = format!(
                "{} {:?}",
                lang.tr("Fonemizador selecionado:", "Phonemizer selected:"),
                mode
            );
        }

        if open_singer_panel {
            self.piano_roll_state.is_maximized = false;
            self.piano_roll_state.show_inspector = true;
            self.right_sidebar_tab = crate::gui::types::RightSidebarTab::SingerTrack;
            self.phonemizer_warning_dismissed = true;
            self.phonemizer_warning_expanded = false;
        }
