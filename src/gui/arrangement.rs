...
pub fn draw_arrangement_view(
    ...
    ui.vertical(|ui| {
        ...
        ui.horizontal(|ui| {
            ui.label(
                egui::RichText::new(" ARRANJO / FAIXAS ")
                    .strong()
                    .size(9.5)
            ...
            let add_btn = egui::Button::new(egui::RichText::new("➕ Nova Track").strong().size(10.5).color(Color32::BLACK))
            ...
            if tracks.len() > 1 {
                let del_btn = egui::Button::new(egui::RichText::new("🗑 Excluir").strong().size(10.5).color(Color32::WHITE))
            ...
            ui.add_space(10.0);
            ui.label(
                egui::RichText::new(format!("Ativa: Track {} ({})", *active_track_index + 1, tracks.get(*active_track_index).map(|t| t.name.as_str()).unwrap_or("Track")))
                    .size(11.0)
                    .color(MelodyneTheme::TEXT_SECONDARY)
            );
        });
        ...
        for idx in 0..tracks.len() {
            ...
            ui.horizontal(|ui| {
                ...
                let card_stroke = if is_active {
                    Stroke::new(1.2, MelodyneTheme::BORDER_GOLD)
                } else {
                    Stroke::new(1.0, MelodyneTheme::BORDER_FINE)
                };

                ui.painter().rect_filled(card_rect, Rounding::same(3.0), card_bg);
                ui.painter().rect_stroke(card_rect, Rounding::same(3.0), card_stroke);

                let color_badge = match idx % 4 {
                    0 => MelodyneTheme::ACCENT_GOLD,
                    1 => MelodyneTheme::ACCENT_CYAN,
                    2 => MelodyneTheme::ACCENT_MAGENTA,
                    _ => Color32::from_rgb(180, 120, 255),
                };

                ui.painter().rect_filled(
                    Rect::from_min_size(card_rect.min + Vec2::new(4.0, 4.0 + track_y_offset), Vec2::new(5.0, track_height - 8.0)),
                    Rounding::same(2.0),
                    color_badge,
                );
                ...
                let mute_bg = if track.mute { MelodyneTheme::ACCENT_MAGENTA } else { MelodyneTheme::BG_HEADER };
                ui.painter().rect_filled(mute_rect, Rounding::same(2.0), mute_bg);
                ui.painter().rect_stroke(mute_rect, Rounding::same(2.0), Stroke::new(1.0, Color32::BLACK));
                ui.painter().text(mute_rect.center(), egui::Align2::CENTER_CENTER, "M", egui::FontId::proportional(10.0), if track.mute { Color32::WHITE } else { MelodyneTheme::TEXT_SECONDARY });

                let solo_resp = ui.allocate_rect(solo_rect, egui::Sense::click());
                if solo_resp.clicked() {
                    track.solo = !track.solo;
                }
                let solo_bg = if track.solo { MelodyneTheme::ACCENT_GOLD } else { MelodyneTheme::BG_HEADER };
                ui.painter().rect_filled(solo_rect, Rounding::same(2.0), solo_bg);
                ui.painter().rect_stroke(solo_rect, Rounding::same(2.0), Stroke::new(1.0, Color32::BLACK));
                ui.painter().text(solo_rect.center(), egui::Align2::CENTER_CENTER, "S", egui::FontId::proportional(10.0), if track.solo { Color32::BLACK } else { MelodyneTheme::TEXT_SECONDARY });
                ...
                egui::ScrollArea::horizontal()
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.painter().rect_stroke(strip_rect, Rounding::ZERO, Stroke::new(1.0, MelodyneTheme::BORDER_FINE));

                        // Draw Timeline Ruler
                        if idx == 0 {
                            let ruler_rect = Rect::from_min_size(strip_rect.min, Vec2::new(strip_rect.width(), ruler_height));
                            ui.painter().rect_filled(ruler_rect, Rounding::ZERO, MelodyneTheme::BG_HEADER);
                            ui.painter().rect_stroke(ruler_rect, Rounding::ZERO, Stroke::new(1.0, MelodyneTheme::BORDER_FINE));
                                ...
                                if x > ruler_rect.min.x && x < ruler_rect.max.x {
                                    ...
                                    if b % 4 == 0 {
                                        ...
                                        ui.painter().line_segment(
                                            ...
                                            Stroke::new(1.0, MelodyneTheme::BORDER_GOLD)
                                        );
                                    }
                                }
                            }
                        ...
                        for part in parts.iter().filter(|p| p.track_index == idx) {
                            ...
                            for note in &part.notes {
                                ...
                                if note_rect.max.x > strip_rect.min.x && note_rect.min.x < strip_rect.max.x {
                                    ui.painter().rect_filled(note_rect, Rounding::same(2.0), color_badge);
                                    ui.painter().rect_stroke(note_rect, Rounding::same(2.0), Stroke::new(1.0, Color32::BLACK));
                                }
                            }
                        }
