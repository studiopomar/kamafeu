...
impl KamafeuStudioApp {
    ...
    pub fn execute_export_wav(&mut self, scope: ExportAudioScope) {
            ...
            .save_file()
        {
            ...
            let mastering_options = MasteringOptions {
                target_sample_rate: self.config.export.sample_rate,
                normalize_peak_db: self
                    .config
                    .export
                    .normalize_audio
                    .then_some(self.config.export.normalize_peak_db),
                head_silence_ms: self.config.export.head_silence_ms,
                tail_silence_ms: self.config.export.tail_silence_ms,
            };
            let dither_mode = DitherMode::from_config_label(&self.config.export.dither_mode);
                ...
                if scope == ExportAudioScope::SeparateTrackStems {
                    ...
                    for (t_idx, track) in project.tracks.iter().enumerate() {
                        ...
                        if track_proj.parts.iter().any(|p| !p.notes.is_empty()) {
                            ...
                            audio.sample_rate = mastering_options.prepare(
                                &mut audio.samples,
                                audio.sample_rate,
                                audio.channels,
                            );
                            report_progress(
                                (t_idx as f32 + 0.75) / (track_count.max(1) as f32),
                                &AudioDiagnostics::analyze(&audio.samples, audio.channels)
                                    .render_log_line(&format!("stem {}", t_idx + 1)),
                            );
                            let _ = AudioExporter::export_audio_with_dither(
                                &track_save_path,
                                &audio.samples,
                                audio.sample_rate,
                                audio.channels,
                    ...
                    report_progress(
                        0.95,
                        &AudioDiagnostics::analyze(&audio.samples, audio.channels)
                            .render_log_line("mix final de stems"),
                    );
                    let result = AudioExporter::export_audio_with_dither(
                        &save_path,
                        &audio.samples,
                        audio.sample_rate,
                        audio.channels,
