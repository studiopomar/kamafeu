    ...
    pub(in crate::gui) fn render_export_defaults_tab(&mut self, ui: &mut egui::Ui) {
        ...
        section_card(
            ...
            |ui| {
                ...
                ui.add_space(4.0);
                ui.horizontal(|ui| {
                    ui.label(lang.tr("Dither PCM:", "PCM Dither:"));
                    for mode in ["TPDF (Triangular)", "Desativado"] {
                        if ui
                            .selectable_label(self.config.export.dither_mode == mode, mode)
                            .clicked()
                        {
                            self.config.export.dither_mode = mode.to_string();
                        }
                    }
                    help_marker(
                        ui,
                        lang.tr(
                            "TPDF é aplicado antes da quantização em WAV/FLAC de 16 ou 24-bit. Arquivos float e RAW não são alterados.",
                            "TPDF is applied before 16- or 24-bit WAV/FLAC quantization. Float and RAW files are unchanged.",
                        ),
                    );
                });

                ui.horizontal(|ui| {
                    ui.label(lang.tr("Silêncio Inicial:", "Head Silence:"));
                    ui.add(
                        egui::Slider::new(&mut self.config.export.head_silence_ms, 0.0..=3000.0)
                            .suffix(" ms"),
                    );
                    help_marker(
                        ui,
                        lang.tr(
                            "Tempo de silêncio inserido no começo do arquivo master.",
                            "Silence inserted at the beginning of the master file.",
                        ),
                    );
                });
                ...
                ui.horizontal_wrapped(|ui| {
                    ui.checkbox(
                        &mut self.config.export.export_stems_default,
                        lang.tr("Usar Stems como Escopo Inicial", "Use Stems as Initial Scope"),
                    );
                    ui.checkbox(
                        &mut self.config.export.include_instrumental,
                        lang.tr("Incluir Instrumental por Padrão", "Include Instrumental by Default"),
                    );
                    help_marker(
                        ui,
                        lang.tr(
                            "Define o escopo sugerido da próxima sessão: stems tem prioridade; sem stems, desmarcar instrumental inicia em acapella. O diálogo de exportação sempre permite alterar a escolha.",
                            "Sets the suggested scope for the next session: stems take priority; without stems, disabling instrumental starts with acapella. The export dialog always allows changing it.",
                        ),
                    );
                });
