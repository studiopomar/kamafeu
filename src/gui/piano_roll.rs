...
pub fn draw_piano_roll(
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.vertical(|ui| {
                        ...
                        let param_tabs = [
                            ...
                            ("Vel. Consoante", ParameterTab::Velocity),
                        ...
                        for (p_name, tab_val) in param_tabs {
                            ...
                            let btn = egui::Button::new(
                                egui::RichText::new(p_name).size(10.0).color(text_color),
                            )
                            .fill(fill_color)
                                    ...
                                    match state.selected_parameter {
                                        ...
                                        ParameterTab::PitchDelta => {
                                            note.expressions.pitch_delta =
                                                (norm_y * 100.0).clamp(-100.0, 100.0);
                                        }
                                        ParameterTab::Gender => {
                                        ...
                                        ParameterTab::Velocity => {
                                            let norm_01 = ((graph_rect.max.y - mpos.y)
                                                / graph_rect.height())
                                                as f64;
                                            note.expressions.consonant_velocity =
                                                (norm_01 * 200.0).clamp(0.0, 200.0);
                                        }
                        ...
                        if x_end >= graph_rect.min.x && x_start <= graph_rect.max.x {
                            ...
                            let node_y = match state.selected_parameter {
                                ParameterTab::Velocity | ParameterTab::Breathiness => {
                                    ...
                                    graph_rect.max.y
                                        - (norm as f32 * (graph_rect.height() - 10.0))
                                        - 5.0
                                }
                                _ => {
                                    let norm = val / max_v;
    ...
    if !state.initial_scrolled {
        ...
        scroll_area = scroll_area
            ...
            .horizontal_scroll_offset(target_x);
    ...
    let scroll_output = scroll_area.show(ui, |ui| {
        ...
        let bar_ms = beat_ms * 4.0;
        let visible_time_start = ((visible_clip.min.x - (rect.min.x + keyboard_width))
            / state.px_per_ms)
            .max(0.0) as f64;
        let visible_time_end = ((visible_clip.max.x - (rect.min.x + keyboard_width))
            / state.px_per_ms)
            .max(0.0) as f64;
        ...
        while time_ms <= total_canvas_ms.min(visible_time_end + grid_step_ms) {
            ...
            if x >= rect.min.x + keyboard_width && x <= rect.max.x {
                ...
                let is_beat = (time_ms % beat_ms).abs() < 1e-2
                    || ((time_ms % beat_ms) - beat_ms).abs() < 1e-2;

                let (line_width, line_color) = if is_bar {
                    (1.8, Color32::from_rgb(110, 85, 160))
                } else if is_beat {
                    (1.2, Color32::from_rgb(70, 52, 105))
                } else {
                    (0.8, Color32::from_rgb(48, 36, 72))
                };
        ...
        for (idx, note) in notes.iter_mut().enumerate() {
            ...
            if !note_rect.intersects(visible_clip) {
                continue;
            }
            ...
            if is_editing_lyric {
                ...
                if commit_lyric_edit.is_none()
                    && (text_lost_focus
                        || is_clicked_outside
                        || ui.input(|i| i.key_pressed(Key::Enter))
                        || ui.input(|i| i.key_pressed(Key::Tab)))
                {
                    let final_lyric = if !candidates.is_empty()
            ...
            for (t_ms, vol) in env_pts.iter() {
                ...
                let px_y =
                    y_bottom - (*vol / 100.0).clamp(0.0, 1.0) as f32 * (state.row_height - 4.0);
            ...
            if let Some(first) = env_screen_pts.first() {
                painter.line_segment(
                    ...
                    Stroke::new(1.5, env_color.linear_multiply(0.5)),
                );
            }
            ...
            for (pt_idx, &pt_pos) in env_screen_pts.iter().enumerate() {
                ...
                let is_hovered = mouse_interact_pos.is_some_and(|m| pt_rect.contains(m));
                let is_dragging = state.dragging_envelope_pt == Some((idx, pt_idx));

                let pt_color = if is_dragging || is_hovered {
                    Color32::WHITE
                } else {
                    env_color
                };
            ...
            if let Some((drag_note_idx, drag_pt_idx)) = state.dragging_envelope_pt {
                if drag_note_idx == idx {
                    if let Some(mpos) = ui.input(|i| i.pointer.latest_pos()) {
                        ...
                        let delta_v = ((y_bottom - mpos.y) / (state.row_height - 4.0) * 100.0)
                            .clamp(0.0, 100.0) as f64;

                        match drag_pt_idx {
                            0 => {
                                note.envelope.p1 = delta_t.max(0.0);
                                note.envelope.v1 = delta_v;
                            }
                            1 => {
                                note.envelope.p2 = (delta_t - note.envelope.p1).max(0.0);
                                note.envelope.v2 = delta_v;
                            }
                            2 => {
                                note.envelope.p3 =
                                    (delta_t - note.envelope.p1 - note.envelope.p2).max(0.0);
                                note.envelope.v3 = delta_v;
                            }
                            3 => {
                                note.envelope.p4 = (note.duration_ms - delta_t).max(0.0);
                                note.envelope.v4 = delta_v;
                            }
                            4 => {
                                note.envelope.p5 =
                                    (delta_t - (note.duration_ms - note.envelope.p4)).max(0.0);
                                note.envelope.v5 = delta_v;
                            }
                            _ => {}
                        }
                    }
                        ...
                        if response.dragged() && state.marquee_start.is_some() {
                                ...
                                (state.marquee_start, state.marquee_current)
                            {
                                ...
                                painter.rect_stroke(
                                    sel_rect,
                                    Rounding::same(2.0),
                                    Stroke::new(1.2, MelodyneTheme::NOTE_SELECTED_GOLD),
                                );
        ...
        // 6.5 Phoneme Drag & Drop Canvas Handler
            ...
            if let Some(mpos) = mouse_interact_pos {
                    ...
                    && mpos.y < grid_end_y
                {
                    ...
                    painter.rect_stroke(
                        hover_rect,
                        Rounding::ZERO,
                        Stroke::new(1.0, Color32::from_rgb(0, 255, 157)),
                    );
        ...
        for key_idx in first_visible_key..last_visible_key {
            ...
            painter.rect_stroke(
                key_rect,
                Rounding::ZERO,
                Stroke::new(1.0, Color32::from_rgb(15, 12, 20)),
            );
                ...
                .show(ui.ctx(), |ui| {
                    egui::Grid::new("prop_grid")
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.label("Velocity MIDI");
                            ui.add(
                                egui::DragValue::new(&mut note.expressions.velocity)
                                    .speed(1.0)
                                    .range(0.0..=200.0),
                            );
                            ui.end_row();
                            ...
                            .on_hover_text(
                                "100% = original; 200% = duas vezes mais rápida; 0% = duas vezes mais lenta",
                            );
                            ui.end_row();
