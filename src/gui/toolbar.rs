...
pub fn draw_toolbar(
    ...
    ui.horizontal(|ui| {
        ui.add_space(4.0);
        ui.label(RichText::new("TOOLS").strong().size(10.0).color(Color32::from_rgb(180, 170, 190)));

        let tools = [
            (EditTool::Pointer, "↖ Pointer [V]"),
            (EditTool::Pencil, "✏ Pencil [N]"),
            (EditTool::PitchDraw, "📈 Pitch Draw [P]"),
            (EditTool::Eraser, "🧹 Eraser [E]"),
        ];
        ...
        ui.label(RichText::new("🔍 X:").size(10.0).color(Color32::from_rgb(180, 170, 190)));
        ...
        ui.label(RichText::new("🔍 Y:").size(10.0).color(Color32::from_rgb(180, 170, 190)));
        if ui.button(RichText::new("-").size(11.0)).clicked() {
