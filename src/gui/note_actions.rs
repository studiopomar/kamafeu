...
/// Enforces the editor's default monophonic timeline without turning very
/// short notes into an overlap again.  A one-millisecond minimum retains a
/// selectable/audible event while ensuring that every following note starts
/// at or after the prior note ends.
pub(crate) fn resolve_monophonic_overlaps(notes: &mut [UNote]) {
    let mut order: Vec<usize> = (0..notes.len()).collect();
    order.sort_by(|&a, &b| notes[a].position_ms.total_cmp(&notes[b].position_ms));

    for pair in order.windows(2) {
        let previous = pair[0];
        let current = pair[1];
        let current_start = notes[current].position_ms;
        let previous_start = notes[previous].position_ms;
        if previous_start + notes[previous].duration_ms > current_start {
            let available_duration = current_start - previous_start;
            if available_duration >= 1.0 {
                notes[previous].duration_ms = available_duration;
            } else {
                // Two notes cannot remain monophonic at the same (or
                // sub-millisecond) onset. Preserve both by placing the latter
                // immediately after the former instead of retaining a hidden
                // overlap.
                notes[current].position_ms = previous_start + notes[previous].duration_ms;
            }
        }
    }
}
...
impl KamafeuStudioApp {
    ...
    pub(crate) fn resolve_note_overlaps(&mut self) {
        resolve_monophonic_overlaps(self.current_notes_mut());
    }
    ...
    pub fn push_history(&mut self) {
        self.undo_manager
            .set_max_history(self.config.workflow.max_undo_steps as usize);
    ...
    pub fn humanize_selection(&mut self) {
        ...
        for &idx in &target_indices {
            if let Some(note) = notes.get_mut(idx) {
                ...
                // The one-click humanizer must not break legato phrases. Time
                // variation remains available in the detailed dialog, but the
                // convenient command deliberately preserves every onset.
    ...
    pub fn fix_overlapping_notes(&mut self) {
        ...
        let before: Vec<(f64, f64)> = notes
            .iter()
            .map(|note| (note.position_ms, note.duration_ms))
            .collect();
        resolve_monophonic_overlaps(notes);
        let fixed_count = notes
            .iter()
            .zip(before)
            .filter(|(note, (position, duration))| {
                note.position_ms != *position || note.duration_ms != *duration
            })
            .count();
    ...
    pub fn rephonemize_all_notes(&mut self) {
        ...
        // If in a Japanese phonemizer mode, convert romaji lyrics directly on notes to hiragana
        let is_japanese_mode = matches!(
            mode,
        ...
        for part in self.project.parts.iter_mut() {
            if is_japanese_mode {
                for note in part.notes.iter_mut() {
                    let trimmed = note.lyric.trim();
                    if !trimmed.is_empty() && trimmed != "+" && trimmed != "R" && trimmed != "r" {
                        let converted = crate::phonemizer::romaji::romaji_to_hiragana(trimmed);
        ...
        if let Some(voicebank) = self.voicebank.as_ref() {
            let layouts = self
                .project
                .parts
                .iter()
                .map(|part| {
                    ...
                    let mut per_note = vec![Vec::new(); part.notes.len()];
                    for phone in phones {
                        if let Some(durations) = per_note.get_mut(phone.note_index) {
                            durations.push(phone.duration_ms.max(0.0));
                        }
                    }
                    per_note
                })
                .collect::<Vec<_>>();
            for (part, per_note) in self.project.parts.iter_mut().zip(layouts) {
                for (note, durations) in part.notes.iter_mut().zip(per_note) {
                    let is_manual_composite = mode == crate::phonemizer::PhonemizerMode::None
                        && note.lyric.contains(['.', ';', ',']);
                    if is_manual_composite && !durations.is_empty() {
                        note.phoneme_durations_ms = durations;
                    }
                }
            }
        }
        ...
        self.is_dirty = true;
        self.transport_state.status_message =
            "Todas as notas foram refonetizadas e suas posições fonéticas foram atualizadas."
                .to_string();
    }
    ...
    pub fn selected_or_all_note_indices(&self) -> Vec<usize> {
        let total = self.current_notes().len();
        if !self.piano_roll_state.selected_note_indices.is_empty() {
            self.piano_roll_state
                .selected_note_indices
                .iter()
                ...
                .filter(|&i| i < total)
                .collect()
        } else if let Some(idx) = self.piano_roll_state.selected_note_index {
            if idx < total {
                vec![idx]
            } else {
                vec![]
    ...
    /// ACPT: Auto-adjust envelope crossfades based on note timing and oto.ini overlaps
    pub fn apply_envelope_acpt(&mut self) {
        self.push_history();
        let target_indices = self.selected_or_all_note_indices();
        if target_indices.is_empty() {
            return;
        }
        let vb = self.voicebank.clone();
        let notes = self.current_notes_mut();
        let mut count = 0;
        ...
        for &idx in &target_indices {
            ...
            let lyric = notes[idx].lyric.clone();
            let pitch = notes[idx].pitch.clone();
            let oto = vb
                .as_ref()
                .and_then(|v| v.find_mapped_entry(&lyric, &pitch));
            let ovl = oto
                .map(|e| e.overlap.max(5.0))
                .unwrap_or(35.0)
                .clamp(5.0, 500.0);

            let note = &mut notes[idx];
            note.envelope.p1 = 0.0;
            note.envelope.p2 = ovl;
            note.envelope.p3 = 35.0;
            note.envelope.p4 = 0.0;
            note.envelope.p5 = 35.0;
            note.envelope.v1 = 0.0;
            note.envelope.v2 = 100.0;
            note.envelope.v3 = 100.0;
            note.envelope.v4 = 100.0;
            note.envelope.v5 = 0.0;
        ...
        self.is_dirty = true;
        self.piano_roll_state.continuous_edit_dirty = true;
        self.piano_roll_state.show_parameters_drawer = true;
        self.piano_roll_state.selected_parameter =
            crate::gui::piano_roll::state::ParameterTab::EnvelopeFadeIn;
        self.transport_state.status_message = format!(
            "ACPT: Envelopes e crossfades ajustados em {} nota(s)",
            count
        );
    }

    /// P2P3: Set attack p2 and decay/release p3 to match adjacent note overlap
    pub fn apply_envelope_p2p3(&mut self) {
        self.push_history();
        let target_indices = self.selected_or_all_note_indices();
        if target_indices.is_empty() {
            return;
        ...
        self.piano_roll_state.selected_parameter =
            crate::gui::piano_roll::state::ParameterTab::EnvelopeFadeIn;
        self.transport_state.status_message =
            format!("P2P3: Transição de overlap ajustada em {} nota(s)", count);
    }

    /// P1P4: Zero p1 and p4 for clean wide boundary crossfading
    pub fn apply_envelope_p1p4(&mut self) {
        self.push_history();
        let target_indices = self.selected_or_all_note_indices();
        let notes = self.current_notes_mut();
        let mut count = 0;
        ...
        self.piano_roll_state.selected_parameter =
            crate::gui::piano_roll::state::ParameterTab::EnvelopeFadeIn;
        self.transport_state.status_message =
            format!("P1P4: Bordas zeradas (p1=0, p4=0) em {} nota(s)", count);
    }

    /// P1P2: Align attack to onset/overlap
    pub fn apply_envelope_p1p2(&mut self) {
        self.push_history();
        let target_indices = self.selected_or_all_note_indices();
        let vb = self.voicebank.clone();
        let notes = self.current_notes_mut();
        ...
        for &idx in &target_indices {
            ...
            let ovl = oto
                .map(|e| e.overlap.max(5.0))
                .unwrap_or(notes[idx].envelope.p2)
                .clamp(5.0, 500.0);
        ...
        let sample_p2 = notes.first().map(|n| n.envelope.p2).unwrap_or(5.0);
        self.piano_roll_state.phoneme_cache.clear();
        self.piano_roll_state.note_phonemes_cache.clear();
        self.is_dirty = true;
        self.piano_roll_state.continuous_edit_dirty = true;
        ...
        self.piano_roll_state.selected_parameter =
            crate::gui::piano_roll::state::ParameterTab::EnvelopeFadeIn;
        self.transport_state.status_message = format!(
            "P1P2: Ataque ajustado (p1=0, p2={:.1}ms) em {} nota(s)",
            sample_p2, count
        );
    }

    /// P2P4: Fit attack to overlap and sustain to note end
    pub fn apply_envelope_p2p4(&mut self) {
        self.push_history();
        let target_indices = self.selected_or_all_note_indices();
        let vb = self.voicebank.clone();
        let notes = self.current_notes_mut();
        ...
        self.piano_roll_state.show_parameters_drawer = true;
        self.piano_roll_state.selected_parameter =
            crate::gui::piano_roll::state::ParameterTab::EnvelopeFadeOut;
        self.transport_state.status_message =
            format!("P2P4: Ataque e sustain ajustados em {} nota(s)", count);
    }

    /// OPT: Optimize all envelopes and crossfades across selected notes
    pub fn apply_envelope_opt(&mut self) {
        self.push_history();
        let target_indices = self.selected_or_all_note_indices();
        if target_indices.is_empty() {
            return;
        ...
        for &idx in &target_indices {
            ...
            let note = &mut notes[idx];
            note.envelope.p1 = 0.0;
            note.envelope.p2 = (dur * 0.25).min(ovl).max(5.0);
            note.envelope.p3 = (dur * 0.1).clamp(0.0, 35.0);
            note.envelope.p4 = 0.0;
            note.envelope.p5 = (dur * 0.2).clamp(5.0, 35.0);
            note.envelope.v1 = 0.0;
            note.envelope.v2 = 100.0;
            note.envelope.v3 = 100.0;
            note.envelope.v4 = 100.0;
        ...
        self.piano_roll_state.selected_parameter =
            crate::gui::piano_roll::state::ParameterTab::EnvelopeFadeIn;
        self.transport_state.status_message = format!(
            "OPT: Envelopes otimizados proporcionalmente em {} nota(s)",
            count
        );
    }

    /// RESET: Reset envelopes of selected notes to standard default
    pub fn apply_envelope_reset(&mut self) {
        self.push_history();
        let target_indices = self.selected_or_all_note_indices();
        let notes = self.current_notes_mut();
        let mut count = 0;
        ...
        self.piano_roll_state.selected_parameter =
            crate::gui::piano_roll::state::ParameterTab::EnvelopeFadeIn;
        self.transport_state.status_message = format!(
            "RESET: Envelopes restaurados para o padrão em {} nota(s)",
            count
        );
    }
}
...
#[cfg(test)]
mod tests {
    use super::resolve_monophonic_overlaps;
    use crate::project::model::UNote;

    #[test]
    fn auto_cut_removes_collisions_even_for_very_short_notes() {
        let mut notes = vec![
            UNote::new("a", "C4", 0.0, 100.0),
            UNote::new("i", "D4", 0.5, 80.0),
            UNote::new("u", "E4", 1.0, 80.0),
        ];

        resolve_monophonic_overlaps(&mut notes);

        for pair in notes.windows(2) {
            assert!(pair[0].position_ms + pair[0].duration_ms <= pair[1].position_ms);
        }
    }
}
