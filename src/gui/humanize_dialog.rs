...
impl Default for HumanizeParams {
    fn default() -> Self {
        Self {
            // Timing is opt-in: a default time offset breaks adjacent notes
            // into audible gaps, which is surprising for a one-click effect.
...
/// A deliberately assertive but bounded vibrato preset for growl-like UTAU
/// phrasing.  It remains an editable vibrato template, not a fake resampler
/// effect, so it works with every supported engine.
pub fn apply_growl_vibrato(notes: &mut [UNote], target_indices: &[usize]) {
    for &idx in target_indices {
        if let Some(note) = notes.get_mut(idx) {
            if note.duration_ms >= 220.0 {
                note.vibrato = VibratoParam {
                    length_pct: 82.0,
                    period_ms: 95.0,
                    depth_cents: 110.0,
                    fade_in_ms: 0.0,
...
pub fn draw_humanize_dialog(
    ...
    let mut apply_growl_clicked = false;
    ...
    .show(ctx, |ui| {
        ui.vertical(|ui| {
            ...
            Frame::none()
                ...
                .show(ui, |ui| {
                    ...
                    if ui
                        .button(
                            RichText::new(lang.tr(
                                "Template Growl (Vibrato Intenso)",
                                "Growl Template (Intense Vibrato)",
                            ))
                            .size(11.0),
                        )
                        .on_hover_text(lang.tr(
                            "Aplica um vibrato rápido, profundo e com drift às notas sustentadas.",
                            "Applies a fast, deep, drifting vibrato to sustained notes.",
                        ))
                        .clicked()
                    {
                        apply_growl_clicked = true;
                    }
                });
            ...
            let has_changes = apply_humanize_clicked
                || apply_vibrato_clicked
                || apply_growl_clicked
                || apply_all_clicked;
            if has_changes {
                on_applied();
            ...
            if apply_growl_clicked {
                apply_growl_vibrato(notes, &target_indices);
            }
...
mod tests {
    ...
    #[test]
    fn default_humanization_preserves_note_onsets() {
        let mut notes = vec![
            UNote::new("a", "C4", 100.0, 400.0),
            UNote::new("ka", "D4", 500.0, 400.0),
        ];
        let original_positions: Vec<_> = notes.iter().map(|note| note.position_ms).collect();

        apply_humanize(&mut notes, &[0, 1], &HumanizeParams::default(), 12345);

        assert_eq!(
            notes
                .iter()
                .map(|note| note.position_ms)
                .collect::<Vec<_>>(),
            original_positions
        );
    }
    ...
    #[test]
    fn growl_template_is_bounded_and_skips_short_notes() {
        let mut notes = vec![
            UNote::new("short", "C4", 0.0, 200.0),
            UNote::new("long", "C4", 200.0, 600.0),
        ];
        apply_growl_vibrato(&mut notes, &[0, 1]);
        assert_eq!(notes[0].vibrato.length_pct, 0.0);
        assert_eq!(notes[1].vibrato.depth_cents, 110.0);
        assert_eq!(notes[1].vibrato.period_ms, 95.0);
    }
}
