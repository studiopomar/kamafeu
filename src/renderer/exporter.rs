...
impl AudioExporter {
    ...
    pub fn export_to_wav<P: AsRef<Path>>(path: P, samples: &[f32], sample_rate: u32) -> Result<(), String> {
        let file = File::create(path.as_ref()).map_err(|e| format!("Failed to create WAV file: {}", e))?;
        ...
        writer.write_all(&16u32.to_le_bytes()).map_err(|e| e.to_string())?; // Subchunk1Size (16 for PCM)
        writer.write_all(&1u16.to_le_bytes()).map_err(|e| e.to_string())?;  // AudioFormat (1 for PCM)
        ...
        for &sample in samples {
            ...
            writer.write_all(&s_i16.to_le_bytes()).map_err(|e| e.to_string())?;
        }
