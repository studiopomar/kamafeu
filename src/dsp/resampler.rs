...
impl Resampler {
    ...
    pub fn render_sample_with_pitch_bend_and_consonant_timing(
        ...
        if target_consonant_samples > 0 && !consonant_slice.is_empty() {
            output.extend(Self::resize_segment_linear(
                consonant_slice,
        ...
        if target_vowel_samples > 0 && !vowel_slice.is_empty() {
            ...
            let lpc_coeffs =
                crate::dsp::lpc::LpcExtractor::extract_lpc_coefficients(vowel_slice, lpc_order);
            let residual_slice =
                crate::dsp::lpc::LpcExtractor::extract_residual(vowel_slice, &lpc_coeffs);
            ...
            let synth_vowel =
                crate::dsp::lpc::LpcExtractor::synthesize(&synth_residual, &lpc_coeffs);
    ...
    fn resize_segment_linear(samples: &[f32], target_len: usize) -> Vec<f32> {
        ...
        let source_span = samples.len().saturating_sub(1) as f64;
        let target_span = target_len - 1;
        (0..target_len)
            .map(|index| {
                let source_position = index as f64 * source_span / target_span as f64;
