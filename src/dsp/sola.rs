...
impl SolaResampler {
        ...
        if target_vowel_samples > 0 && !vowel_slice.is_empty() {
            ...
            if target_join_samples > 0 && output.len() >= target_join_samples {
                ...
                for index in 0..target_join_samples {
                    ...
                    let w_consonant = 1.0 - smooth_phase;
                    let w_vowel = smooth_phase;
    ...
    fn interpolate_pitch_marks(
        marks: &[usize],
        source_position: f64,
        fallback_period: f64,
    ) -> (f64, f64, f64, f64) {
        if marks.is_empty() {
            return (source_position, source_position, 0.0, fallback_period);
        }
        if marks.len() == 1 {
            let p = Self::local_period(marks, 0, fallback_period);
            return (marks[0] as f64, marks[0] as f64, 0.0, p);
        }

        let target = source_position.max(0.0);
        let first_mark = marks[0] as f64;
        let last_mark = marks[marks.len() - 1] as f64;

        if target <= first_mark {
            let period = Self::local_period(marks, 0, fallback_period);
            return (first_mark, first_mark, 0.0, period);
        }
        if target >= last_mark {
            let last_idx = marks.len() - 1;
            let period = Self::local_period(marks, last_idx, fallback_period);
            return (last_mark, last_mark, 0.0, period);
        }

        let target_usize = target.floor() as usize;
        let idx = match marks.binary_search(&target_usize) {
            Ok(i) => i,
            Err(i) => i.saturating_sub(1),
        };
        let idx = idx.min(marks.len() - 2);

        let m0 = marks[idx] as f64;
        let m1 = marks[idx + 1] as f64;
        let span = (m1 - m0).max(1.0);
        let alpha = ((target - m0) / span).clamp(0.0, 1.0);

        let period_0 = Self::local_period(marks, idx, fallback_period);
        let period_1 = Self::local_period(marks, idx + 1, fallback_period);
        let period = period_0 * (1.0 - alpha) + period_1 * alpha;
    ...
    fn render_consonant(
        ...
        preserved
            ...
            .map(|(index, (preserved, pitched))| {
                ...
                let smooth_phase = phase * phase * (3.0 - 2.0 * phase);
                preserved * (1.0 - smooth_phase) + pitched * smooth_phase
            })
            .collect()
    }
    ...
    fn render_vowel_psola(
        ...
        let base_target_period = (sample_rate as f64 / target_pitch_freq.clamp(40.0, 1_400.0))
        ...
        while output_center < max_output_center {
            ...
            let target_period = (sample_rate as f64 / cur_target_freq)
                .clamp(minimum_target_period, maximum_target_period);
            let clamped_out_pos =
                (output_center.max(0.0).round() as usize).min(target_samples.saturating_sub(1));
            ...
            for output_index in first..=last {
                ...
                let sample_val = if alpha <= 1e-4 {
                    let p0 = mark_0 + delta;
                    if p0 >= 0.0 && p0 < v_len as f64 {
                        Self::cubic_sample(vowel, p0)
                    } else {
                        continue;
                    }
                } else if alpha >= 1.0 - 1e-4 {
                    let p1 = mark_1 + delta;
                    if p1 >= 0.0 && p1 < v_len as f64 {
                        Self::cubic_sample(vowel, p1)
                    } else {
                        continue;
                    }
                } else {
                    let p0 = mark_0 + delta;
                    let p1 = mark_1 + delta;
                    let v0 = if p0 >= 0.0 && p0 < v_len as f64 {
                        Self::cubic_sample(vowel, p0)
                    } else {
                        0.0
                    };
                    let v1 = if p1 >= 0.0 && p1 < v_len as f64 {
                        Self::cubic_sample(vowel, p1)
                    } else {
                        0.0
                    };
                    v0 * (1.0 - alpha as f32) + v1 * (alpha as f32)
                };
