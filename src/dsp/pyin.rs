...
impl PitchExtractor {
    pub fn extract_pitch_and_gci(vowel_slice: &[f32], sample_rate: u32) -> PitchResult {
        Self::extract_pitch_and_gci_with_range(vowel_slice, sample_rate, 50.0, 1_000.0)
    }

    pub fn extract_pitch_and_gci_with_range(
        vowel_slice: &[f32],
        sample_rate: u32,
        min_f0_hz: f64,
        max_f0_hz: f64,
        ...
        let min_f0_hz = min_f0_hz.clamp(20.0, 2_000.0);
        let max_f0_hz = max_f0_hz.clamp(min_f0_hz + 1.0, 4_000.0);
        let min_period = (sample_rate as f64 / max_f0_hz) as usize;
        let max_period = (sample_rate as f64 / min_f0_hz) as usize;
        ...
        if vowel_slice.len() < frame_size {
            ...
            let p0 = f0s
                ...
                .zip(voiced)
                .find_map(|(f0, voiced)| {
                    (voiced && f0 > 0.0).then_some((sample_rate as f32 / f0) as usize)
                })
                .unwrap_or_else(|| (sample_rate as usize / 150).max(16));
