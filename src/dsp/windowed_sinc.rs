...
impl WindowedSincResampler {
    ...
    /// * `offset_ms` - Offset from the start of the sample in milliseconds (`oto.ini` offset)
    /// * `consonant_ms` - Duration of the consonant part in milliseconds (`oto.ini` cutoff)
    ...
    pub fn render_sample_with_pitch_bend(
        ...
        // Use the shared oto.ini bounds function to determine the source region
