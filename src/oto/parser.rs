...
impl OtoParser {
    ...
    pub fn parse_bytes(bytes: &[u8]) -> HashMap<String, OtoEntry> {
        if let Ok(text) = std::str::from_utf8(bytes) {
            return Self::parse_str(text.trim_start_matches('\u{feff}'));
        }
...
"#;
    ...
    #[test]
    fn parses_utf8_aliases_without_mojibake() {
        let map = OtoParser::parse_bytes("voz.wav=céu,1,2,3,4,5\n".as_bytes());
        assert!(map.contains_key("céu"));
    }
}
