...
pub trait WavtoolDriver: Send + Sync {
    ...
    /// phase alignment. Simple and external wavtools own their placement and
    /// must never be shifted again by the track mixer.
    fn phase_alignment(&self) -> bool {
        false
    }
...
impl KnownWavtool {
    ...
    pub const fn label(self) -> &'static str {
        match self {
            Self::Galapagos => "Galapagos Convergence (nativo)",
            Self::WavtoolYawu => "Kamafeu Simple / yawu externo",
    ...
    pub const fn executable_names(self) -> &'static [&'static str] {
        match self {
            Self::Galapagos => &[
                ...
                "galapagos.exe",
                "openutau-wavtool",
                "openutau-wavtool.exe",
                "worldline",
                "worldline.exe",
                "worldline-wavtool",
                "worldline-wavtool.exe",
            ],
...
pub struct GalapagosWavtoolDriver;
pub type OpenUtauWavtoolDriver = GalapagosWavtoolDriver;

impl WavtoolDriver for GalapagosWavtoolDriver {
    fn name(&self) -> &str {
        "Galapagos Convergence (Nativo)"
    }
    ...
    fn process_note(
        ...
        // Apply Galapagos equal-power cosine S-curve envelope.
...
impl WavtoolDriver for WavtoolYawuDriver {
    ...
    fn name(&self) -> &str {
        if self.uses_integrated_backend() {
            "Kamafeu Simple (wrapper de compatibilidade yawu)"
        } else {
...
mod tests {
    ...
    #[test]
    fn galapagos_driver_applies_smooth_s_curve_envelope() {
        let driver = GalapagosWavtoolDriver;
        assert_eq!(driver.name(), "Galapagos Convergence (Nativo)");
        assert!(driver.phase_alignment());
        assert!(!driver.consumes_skip_over());
        assert!(driver.phrase_executable().is_none());
