...
pub fn draw_copaiba_toolkit_ui(app: &mut CopaibaToolkitApp, ui: &mut egui::Ui) {
    ui.vertical(|ui| {
        ...
        ui.horizontal(|ui| {
            // 1. Left Sidebar Panel: Responsive Full Height Alias & WAV List
            ...
            // 2. Center Panel: Stacked 3-Waveform View (Previous - Active Center - Next)
            ...
            ui.allocate_ui_with_layout(Vec2::new(center_w, full_avail_h), egui::Layout::top_down(egui::Align::LEFT), |ui| {
                ...
                // --- 2A. TOP: PREVIOUS WAVEFORM PREVIEW ---
                ui.horizontal(|ui| {
                ...
                prev_painter.rect_stroke(prev_rect, Rounding::same(4.0), Stroke::new(1.0, Color32::from_rgb(35, 28, 48)));
                ...
                if let Some(ref p_entry) = prev_entry {
                    ...
                    if let Some((ref samples, _)) = app.load_wav_file(&p_entry.wav_filename) {
                        draw_waveform_peak_envelope(&prev_painter, prev_rect, samples, Color32::from_rgb(0, 150, 100), app.zoom_y * 0.8);
                    }
                }

                ui.add_space(4.0);

                // --- 2B. CENTER: ACTIVE WAVEFORM MAIN EDITOR ---
                let (canvas_rect, canvas_resp) = ui.allocate_exact_size(Vec2::new(center_w, main_h.max(120.0)), Sense::click_and_drag());
                let painter = ui.painter_at(canvas_rect);

                // Deep Neon Canvas Box
                painter.rect_filled(canvas_rect, Rounding::same(4.0), Color32::from_rgb(10, 8, 18));
                painter.rect_stroke(canvas_rect, Rounding::same(4.0), Stroke::new(1.5, Color32::from_rgb(0, 255, 157)));
                ...
                if let (Some((ref samples, sr)), Some(idx)) = (&app.loaded_waveform, sel_idx) {
                    if idx < app.config.entries.len() {
                        ...
                        // Draw Main Peak Envelope Waveform
                        draw_waveform_peak_envelope(&painter, Rect::from_min_max(Pos2::new(canvas_rect.min.x, wave_area_min_y), canvas_rect.max), samples, Color32::from_rgb(0, 255, 160), zoom_y);
                        ...
                        // Optional Final Tail Region (Purple Line and Purple Shaded Handle!)
                        if let Some(xt) = x_tail {
                            ...
                            painter.rect_filled(tail_rect, Rounding::ZERO, Color32::from_rgba_premultiplied(180, 50, 255, 45));
                            painter.line_segment([Pos2::new(xt, wave_area_min_y), Pos2::new(xt, canvas_rect.max.y)], Stroke::new(2.5, Color32::from_rgb(190, 60, 255)));
                            painter.text(Pos2::new(xt + 3.0, wave_area_min_y + 4.0), egui::Align2::LEFT_TOP, "Cauda Final", egui::FontId::proportional(10.0), Color32::from_rgb(200, 80, 255));
                        }

                        // Render Main Marker Lines & Labels
                        ...
                        // Interactive Mouse Dragging for ALL 5 Markers (including Cauda Final!)
                        if let Some(mpos) = canvas_resp.interact_pointer_pos() {
                            if canvas_resp.dragged() || canvas_resp.clicked() {
                                ...
                                let min_dist = dist_offset.min(dist_cons).min(dist_cutoff).min(dist_tail);
                ...
                // --- 2C. BOTTOM: NEXT WAVEFORM PREVIEW ---
                ui.horizontal(|ui| {
                ...
                next_painter.rect_stroke(next_rect, Rounding::same(4.0), Stroke::new(1.0, Color32::from_rgb(35, 28, 48)));
                ...
                if let Some(ref n_entry) = next_entry {
                    ...
                    if let Some((ref samples, _)) = app.load_wav_file(&n_entry.wav_filename) {
                        draw_waveform_peak_envelope(&next_painter, next_rect, samples, Color32::from_rgb(0, 150, 100), app.zoom_y * 0.8);
                    }
                }
            });
            ...
            // 3. Right Sidebar Panel: Sliders & Controls (Responsive Full Height Scroll Area)
                    ...
                    .show(ui, |ui| {
                        ...
                        if let (Some(idx), Some(duration_ms)) = (sel_idx, wav_duration_ms) {
                            if idx < app.config.entries.len() {
                                ...
                                // Optional Diphthong Final Tail Section with Slider!
