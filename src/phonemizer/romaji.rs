/// Bidirectional Romaji ↔ Hiragana conversion for UTAU phonemizer.
///
/// Supports standard Japanese CV syllables: basic kana, voiced (dakuten),
/// semi-voiced (handakuten), and combination syllables (きゃ, しゃ, etc.)
...
pub fn lyric_candidates(lyric: &str) -> Vec<String> {
    ...
    if let Some(space_idx) = trimmed.find(|c| c == ' ' || c == '_') {
    ...
    let is_ascii = trimmed.chars().all(|c| c.is_ascii());
