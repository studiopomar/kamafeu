            ...
            match mode {
                PhonemizerMode::BasicCV => {
                    ...
                    if is_phrase_start {
                        ...
                        // CV(VV) banks may provide vowel-to-vowel aliases.
                        // Prefer those transitions, while retaining ordinary
                        // CV lookup as the fallback for standard CV banks.
                        let mut candidates = vec![alias.clone()];
                        if let (Some(pv), Some(cv)) = (prev_vowel, cur_vowel) {
                            candidates.insert(0, format!("{} {}", pv, cv));
                            candidates.insert(1, format!("{}{}", pv, cv));
                            candidates.insert(2, format!("{}_{}", pv, cv));
                        }
                        if let Some(found) = Self::find_oto_candidate(vb, &candidates, &note.pitch)
                        {
                            alias = found;
                        }
                    }
                        ...
                        if is_next_adjacent && !PLAIN_VOWELS.contains(&next_exp.lyric) {
                                ...
                                (cur_vowel, Self::extract_consonant(next_exp.lyric))
                            {
                                ...
                                if let Some(vc_alias) = Self::find_oto_candidate(
                                    ...
                                    .and_then(|alias| {
                                        vb.find_mapped_entry(&alias, &next_exp.note.pitch)
                                    });

                                    if let Some(oto) = oto_entry {
                                        if oto.overlap < 0.0 {
                                            vc_length_ms =
...
mod tests {
    ...
    #[test]
    fn test_cv_vv_alias_is_preferred_when_available() {
        let mut vb = build_test_voicebank();
        vb.entries.insert(
            "a い".to_string(),
            OtoEntry::new(
        ...
        let notes = vec![
            UNote::new("あ", "C4", 0.0, 400.0),
            UNote::new("い", "C4", 400.0, 400.0),
        ];
        let phones = JapanesePhonemizer::apply_japanese(&notes, &vb, PhonemizerMode::BasicCV);
        assert_eq!(phones[1].lyric, "a い");
    }

    #[test]
    fn test_cvvc_romaji_notes_converted_and_matched() {
        let vb = build_test_voicebank();
        // Notes entered in Romaji: "ka", "sa"
        let notes = vec![
        ...
        let phones = JapanesePhonemizer::apply_japanese(&notes, &vb, PhonemizerMode::CVVC);
        assert_eq!(phones.len(), 3);
        assert_eq!(phones[0].lyric, "- か", "Romaji 'ka' should match '- か'");
        assert_eq!(phones[1].lyric, "a s", "VC should be 'a s'");
        assert_eq!(phones[2].lyric, "さ", "Romaji 'sa' should match 'さ'");
    }

    #[test]
    fn test_cvvc_hiragana_vc_matching() {
        let mut vb = build_test_voicebank();
        vb.entries.remove("a s");
        vb.entries.insert(
            "あ s".to_string(),
            OtoEntry::new(
                "as.wav".to_string(),
