        ...
        if is_starting && cc.is_empty() {
            ...
            for i in 0..cc.len().saturating_sub(1) {
                ...
                if text10 != "-" && text11 != "-" {
                    ...
                    {
                        let text12 = Self::resolve_cluster_alias(vb, pitch, text10, text11);
            ...
            if !trailing_coda.is_empty() {
                if let Some(pv) = prev_vowel.as_deref() {
                    ...
                    for alias in end_aliases {
                        ...
                        let stretch = crate::phonemizer::consonant_velocity_time_scale(
                            note.expressions.consonant_velocity,
                        );
                        let coda_dur = vb
                            .find_mapped_entry(&final_alias, &note.pitch)
                            .map(|oto| {
                                if oto.overlap < 0.0 {
                            ...
                            .unwrap_or_else(|| Self::get_transition_basic_length_ms(&final_alias))
                            .mul_add(stretch, 0.0)
                            .max(5.0)
                            .min((note.duration_ms * 0.5).max(5.0));
...
mod tests {
    ...
    #[test]
    fn cluster_transition_uses_the_voicebanks_trailing_variant() {
        ...
        let voicebank = Voicebank {
            ...
            name: "BRAPA cluster test".to_string(),
            author: String::new(),
        ...
        let aliases = VccvBrapaPhonemizer::process_syllable(
            Some("e"),
        ...
        assert!(aliases.iter().any(|alias| alias == "z m-"));
    }
}
