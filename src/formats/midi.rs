...
impl MidiFormat {
    ...
    pub fn parse_bytes(bytes: &[u8]) -> Result<UProject, Box<dyn std::error::Error>> {
        ...
        if ticks_per_beat == 0 {
            return Err(Error::new(ErrorKind::InvalidData, "divisão temporal MIDI é zero").into());
        }

        let tempo_map = Self::tempo_map(&smf);
        if tempo_map
            .iter()
            .any(|point| point.microseconds_per_beat == 0)
        {
            return Err(Error::new(ErrorKind::InvalidData, "evento de tempo MIDI é zero").into());
        }
