...
impl UstFormat {
    ...
    pub fn parse_str(content: &str) -> Result<UProject, Box<dyn std::error::Error>> {
        ...
        let mut current_preutterance: Option<f64> = None;
        let mut current_voice_overlap: Option<f64> = None;
        let mut current_start_point: Option<f64> = None;
        ...
        for line in content.lines() {
            ...
            if line.starts_with("[#") && line.ends_with(']') {
                    ...
                    && section_name != "TRACKEND"
                {
                        ...
                        && current_lyric != "r"
                    {
                        ...
                        note.expressions.preutter_override_ms = current_preutterance;
                        note.expressions.overlap_override_ms = current_voice_overlap;
                        note.expressions.start_point_ms = current_start_point;
                    ...
                    current_flags.clear();
                    current_preutterance = None;
                    current_voice_overlap = None;
            ...
            if parts.len() == 2 {
                ...
                match key {
                    ...
                    "PreUtterance" => {
                        current_preutterance = val.replace(',', ".").parse::<f64>().ok();
                    }
                    "VoiceOverlap" => {
                        current_voice_overlap = val.replace(',', ".").parse::<f64>().ok();
                    }
                    "StartPoint" => {
                        current_start_point = val.replace(',', ".").parse::<f64>().ok();
                    }
                    "VBR" => {
                        let values = val
                            .split(',')
    ...
    pub fn to_ust_string(project: &UProject) -> String {
        ...
        for (idx, note) in notes.iter().enumerate() {
            ...
            if let Some(value) = note.expressions.preutter_override_ms {
                out.push_str(&format!("PreUtterance={value:.3}\n"));
            }
            if let Some(value) = note.expressions.overlap_override_ms {
                out.push_str(&format!("VoiceOverlap={value:.3}\n"));
            }
            if let Some(value) = note.expressions.start_point_ms {
                out.push_str(&format!("StartPoint={value:.3}\n"));
            }
...
"#;
        ...
        assert_eq!(note.expressions.preutter_override_ms, Some(84.5));
        assert_eq!(note.expressions.overlap_override_ms, Some(-12.0));
        assert_eq!(note.expressions.start_point_ms, Some(7.5));
        ...
        assert!(exported.contains("PreUtterance=84.500"));
        assert!(exported.contains("VoiceOverlap=-12.000"));
        assert!(exported.contains("StartPoint=7.500"));
