...
use crate::project::model::{
    UNote, UPhonemeOverride, UPitchBend, UPitchBendPoint, UProject, UTrack, UVoicePart,
};
use std::fs;
use std::path::Path;
            ...
            for (part_index, part_val) in parts_seq.iter().enumerate() {
                ...
                if let Some(notes_seq) = part_val.get("notes").and_then(|v| v.as_sequence()) {
                    for note_val in notes_seq {
                        ...
                        u_note.phonemizer_override = note_val
                            .get("phonemizer")
                            .and_then(|value| value.as_str())
                            .and_then(Self::openutau_name_to_phonemizer_mode)
                            .map(|mode| format!("{mode:?}"));

                        if let Some(overrides) = note_val
                            .get("phonemeOverrides")
                            .or_else(|| note_val.get("phoneme_overrides"))
                            .and_then(|value| value.as_sequence())
                        {
                            let number = |value: &yaml_serde::Value, camel: &str, snake: &str| {
                                value
                                    .get(camel)
                            ...
                            for override_value in overrides {
                                ...
                                u_note.phoneme_overrides.push(UPhonemeOverride {
                                    ...
                                    offset_ms: number(override_value, "offset", "offset")
                                        .map(|ticks| ticks * ms_per_tick),
                                    preutter_delta_ms: number(
                                        override_value,
                                        "preutterDelta",
                                        "preutter_delta",
                                    ),
                                    overlap_delta_ms: number(
                                        override_value,
                                        "overlapDelta",
                                        "overlap_delta",
                                    ),
                        ...
                        if let Some(expr_seq) = note_val
                            .get("phonemeExpressions")
                            .or_else(|| note_val.get("phoneme_expressions"))
                            .or_else(|| note_val.get("expressions"))
                            .and_then(|v| v.as_sequence())
                        {
                            for expr in expr_seq {
                                if let (Some(abbr), Some(val_num)) = (
                                    ...
                                    if expression_index > 0 {
                                        let override_index = u_note
                                            .phoneme_overrides
                                            .iter()
                                            .position(|item| item.index == expression_index)
                                            .unwrap_or_else(|| {
                                                u_note.phoneme_overrides.push(UPhonemeOverride {
                                                    index: expression_index,
                                                    ..UPhonemeOverride::default()
                                                });
                                                u_note.phoneme_overrides.len() - 1
                                            });
                                        let item = &mut u_note.phoneme_overrides[override_index];
                                        match abbr {
                                            "vel" | "con_vel" | "consonant_velocity" => {
                                                item.velocity = Some(val_num)
                                            }
                                            "dyn" => item.dynamics = Some(val_num),
                                            "bre" | "bsh" => item.breathiness = Some(val_num),
                                            "gen" | "g" => item.gender = Some(val_num),
                                            "pitd" => item.pitch_delta = Some(val_num),
                                            "vol" => item.volume = Some(val_num),
                                            "atk" => item.attack = Some(val_num),
                                            "dec" => item.decay = Some(val_num),
                                            "mod" => item.modulation = Some(val_num),
                                            _ => {}
                                        }
                                        continue;
                                    }
                ...
                let notes = part
                    ...
                    .map(|(note_index, note)| {
                        ...
                        let phoneme_overrides = note
                            ...
                            .collect::<Vec<_>>();
                        let note_phonemizer = note
                            .phonemizer_override
                            .as_deref()
                            .and_then(crate::phonemizer::PhonemizerMode::from_name)
                            .map(Self::phonemizer_mode_to_openutau_name);
                        let mut phoneme_expressions = vec![
                            serde_json::json!({ "index": 0, "abbr": "vel", "value": note.expressions.consonant_velocity }),
                            serde_json::json!({ "index": 0, "abbr": "dyn", "value": note.expressions.dynamics }),
                            serde_json::json!({ "index": 0, "abbr": "bre", "value": note.expressions.breathiness }),
                            serde_json::json!({ "index": 0, "abbr": "gen", "value": note.expressions.gender }),
                            serde_json::json!({ "index": 0, "abbr": "pitd", "value": note.expressions.pitch_delta }),
                            serde_json::json!({ "index": 0, "abbr": "vol", "value": note.expressions.volume }),
                            serde_json::json!({ "index": 0, "abbr": "atk", "value": note.expressions.attack }),
                            serde_json::json!({ "index": 0, "abbr": "dec", "value": note.expressions.decay }),
                            serde_json::json!({ "index": 0, "abbr": "mod", "value": note.expressions.modulation }),
                        ];
                        for item in &note.phoneme_overrides {
                            for (abbr, value) in [
                                ("vel", item.velocity),
                                ("dyn", item.dynamics),
                                ("bre", item.breathiness),
                                ("gen", item.gender),
                                ("pitd", item.pitch_delta),
                                ("vol", item.volume),
                                ("atk", item.attack),
                                ("dec", item.decay),
                                ("mod", item.modulation),
                                ...
                                if let Some(value) = value {
                                    phoneme_expressions.push(serde_json::json!({
                                        "index": item.index,
                                        "abbr": abbr,
                                        "value": value,
                                    }));
                        ...
                        serde_json::json!({
                            ...
                            "phonemeExpressions": phoneme_expressions,
                        })
                    })
                    .collect::<Vec<_>>();
                serde_json::json!({
...
mod tests {
    ...
    #[test]
    fn imports_openutau_phoneme_overrides_and_note_phonemizer() {
        let source = r#"
ustx_version: "0.6"
bpm: 120
tracks:
  ...
  - name: Part
    ...
    notes:
      - position: 0
        ...
        tone: 60
        lyric: test
        phonemizer: OpenUtau.Plugin.Builtin.PortugueseBrapaVCCVPhonemizer
        phonemeOverrides:
          - index: 1
            phoneme: "a t"
            offset: -48
...
"#;
        ...
        assert_eq!(
            note.phonemizer_override.as_deref(),
            Some("PortugueseBrapaVCCV")
        );
        assert_eq!(note.phoneme_overrides.len(), 1);
