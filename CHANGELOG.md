...
- Aliases manuais compostos agora usam a preutterance do `oto.ini` para posicionar pontes, em vez de dividir notas longas em partes iguais.
- Overlap negativo e `tailIntrude` compatíveis com OpenUtau são preservados em toda a geometria do envelope.
- Overrides por fonema, expressões, fonemizador por nota e deltas de preutterance/overlap passam a fazer round-trip em USTX; UST preserva `PreUtterance`, `VoiceOverlap` e `StartPoint`.
- Overrides de fonemizador mantêm o contexto dos fonemas vizinhos; fallbacks artificiais de 130/135 ms foram removidos dos modos CVVC/VCCV.
- Alinhamento de fase ficou restrito ao concatenador Galapagos Convergence; wavtools simples e externos não recebem um segundo deslocamento temporal no mixer.
- Quando uma amostra está ausente dentro de uma frase, a vogal anterior é preservada até o próximo fonema válido, evitando buracos nas pontes e mantendo o diagnóstico no log.
- Cache do resampler migrado para o esquema v4 para impedir reutilização de fragmentos produzidos com a temporização anterior.

### Validação
- Adicionados testes de aliases compostos baseados em `oto.ini`, overlap negativo e importação de overrides/expressões do OpenUtau.
- Validado render completo de um projeto BRAPA VCCV real com 833 notas, além de casos isolados de VC e final de palavra.

### Terminologia
- Padronizadas interface, documentação, mensagens de renderização e comentários técnicos para usar exclusivamente `oto.ini`.
